; Modelo basado en agentes para simular encharcamientos en la Ciudad de México
; considerando las elevaciones, dada una altura del agua y una tasa de lluvia
; Octubre 20, 2021
; Yazmín Yolanda Sandoval García
; Edmundo Pacheco Blas
...
to setup
  ...
  set scall-dataset gis:load-dataset "coords-scall.shp"
  gis:set-world-envelope (gis:envelope-union-of (gis:envelope-of elevation-dataset)
                                                (gis:envelope-of scall-dataset))
  display-elevation-in-patches
  set patches-with-elevation patches with [(elevation <= 0) or (elevation >= 0)]
...
to display-scall
  ...
  foreach gis:feature-list-of scall-dataset
  [
    vector-feature -> gis:draw vector-feature 0.5
  ]
end
